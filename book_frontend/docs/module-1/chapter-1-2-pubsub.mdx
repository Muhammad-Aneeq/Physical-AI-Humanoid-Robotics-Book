...
**Publish-Subscribe Pattern** enables decoupled, asynchronous communication in robotics. Publishers send messages to topics without knowing who receives them; subscribers receive messages via callback functions without knowing who sends them. This decoupling allows adding new nodes without modifying existing code—critical for complex multi-sensor robot systems.

**Benefits**: (1) Decoupling—nodes don't depend on each other's implementation, (2) Scalability—add unlimited subscribers without changing publisher, (3) Asynchronous—publisher doesn't wait for processing, (4) Many-to-many—multiple publishers and subscribers per topic.
...
**Key difference**: `create_subscription()` takes a callback function that executes automatically when messages arrive—event-driven programming.
