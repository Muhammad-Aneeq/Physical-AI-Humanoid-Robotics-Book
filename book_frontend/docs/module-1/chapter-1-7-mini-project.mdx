...
**Project Overview:**
Build a complete sensor tower system demonstrating Module 1 mastery—custom messages, publishers, subscribers, multi-node communication, and visualization.
...
**Phase 2: Publisher Nodes** ✅
- [ ] `temperature_publisher`: Publishes to `/sensors/temperature` at 2 Hz (range 20-30°C)
...
**Phase 3: Monitor Node** ✅
- [ ] Subscribes to all 3 sensor topics
- [ ] Implements 3 callback functions (one per sensor)
- [ ] Logs received data in format: `"[TEMP] 23.5°C from sensor_temp_01 at timestamp X"`
...
- [ ] `rqt_graph` shows 3 publishers → 3 topics → 1 monitor subscriber
- [ ] Monitor logs display data from all sensors

**Learning Outcomes Demonstrated:**
✅ Custom message design and generation
✅ Publisher node implementation with timers
✅ Subscriber node with multiple callbacks
✅ Multi-topic communication patterns
✅ ROS 2 CLI debugging tools (topic list/echo/hz)
...
3. Add alarm node that publishes warning when temperature > 28°C
...
- Topic rates match specifications (±10%)
