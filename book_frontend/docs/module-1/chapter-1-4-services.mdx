...
**Services** provide synchronous request-response communication—clients send requests and block waiting for server responses. Unlike topics (one-to-many, async, streaming), services are one-to-one, synchronous, and on-demand.

**Topics vs Services Decision Matrix:**
- ✅ **Topics**: High-frequency streaming (camera 30fps, LiDAR scans), real-time sensor data, many subscribers needed, fire-and-forget
- ✅ **Services**: One-time queries ("What's battery level?"), commands needing confirmation ("Reset odometry"), computed results ("Calculate path"), exactly one server required
