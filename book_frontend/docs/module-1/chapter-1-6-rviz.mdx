...
1. **URDF** (robot description) → Defines geometry, links, joints
2. **Joint states** (current angles) → Defines robot pose at any moment
...
**Workflow:**
```
URDF + /joint_states → robot_state_publisher → TF transforms → RViz2 visualization
```
...
**joint_state_publisher_gui** provides sliders for each joint. Moving sliders publishes to `/joint_states` → robot_state_publisher computes forward kinematics → RViz2 renders updated pose. Essential for testing joint limits and debugging URDF.
