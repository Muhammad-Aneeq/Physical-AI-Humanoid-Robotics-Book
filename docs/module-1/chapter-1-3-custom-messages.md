...
**Custom messages** let you define exactly the data structure your application needs—with type safety, semantic clarity, and efficient serialization. In this chapter, you'll learn to design, build, and use custom ROS 2 message types.
...
✅ **Custom message package** (`custom_msgs`) with `.msg` files
✅ **SensorData.msg**: Temperature, humidity, timestamp
✅ **IMU.msg**: Acceleration, gyroscope, orientation (9-axis sensor)
✅ **Publisher/subscriber** using your custom messages
...
```python
msg = String()
msg.data = "temp:23.5,humidity:65.2"  # ❌ Parsing nightmare!
...
```python
msg = SensorData()  # ✅ Structured, type-safe
...
✅ **No standard type exists**: Your sensor has unique data (e.g., soil moisture + pH + light)
✅ **Combining multiple standard types**: Want temperature + humidity + pressure in one message
✅ **Application-specific data**: Game scores, inventory counts, custom robot states
...
❌ Bad: `msg.val1`, `msg.val2`
✅ Good: `msg.temperature`, `msg.humidity`
...
❌ `float64` for temperature (8 bytes)
✅ `float32` for temperature (4 bytes, 0.0001°C precision sufficient)

❌ `int64` for sensor count (max value 100)
✅ `uint8` for sensor count (0-255 range)
...
🎓 **Custom messages** provide type-safe, efficient, and semantic data structures for application-specific needs.

🎓 **`.msg` files** define message structure using primitive types (int32, float32, string, bool), arrays, and nested messages.

🎓 **Message generation** (`rosidl_generate_interfaces`) creates Python/C++ classes from `.msg` files during `colcon build`.

🎓 **Best practices**: Use descriptive names, include units in comments, add timestamps, choose smallest sufficient types.

🎓 **Separate package** for messages enables reuse across multiple nodes and projects.

---

## What's Next?

You've mastered asynchronous publish-subscribe communication. But what about **synchronous request-response patterns**—like asking a robot "What's your battery level?" and waiting for an answer?
...
**Continue to** → [Chapter 1.4: Services](./chapter-1-4-services.md)
...
**Next Chapter**: [1.4 Services](./chapter-1-4-services.md)
