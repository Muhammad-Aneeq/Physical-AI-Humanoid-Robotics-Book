...
You've mastered **topics** for continuous data streams—camera images flowing at 30 Hz, LiDAR scans at 10 Hz. But what about one-time operations?

- "What's the robot's battery level?" → Need a response
- "Reset odometry to zero" → Need confirmation it happened
- "Calculate inverse kinematics for this pose" → Need computed result

For these **request-response** scenarios, ROS 2 provides **services**—synchronous, blocking communication where a client asks and a server answers.
...
✅ **Service server**: Responds to `AddTwoInts` requests (a + b = sum)
✅ **Service client**: Sends requests and waits for responses
✅ **Custom service**: `SetMotorSpeed` with input validation
✅ **Decision framework**: Topics vs Services trade-offs
...
- **Example**: "Get battery level" → Returns percentage
...
| Emergency stop button | ✅ | ❌ (latency critical) |
| Save map to disk | ❌ | ✅ |

**Rule of thumb**:
- **High-frequency, real-time data** → Topics
- **One-time queries or commands** → Services
- **Need confirmation/result** → Services
- **Many subscribers** → Topics
...
**Success!** You've made your first service call. 🎉
...
✅ **High frequency**: Data updated >1 Hz (sensors, odometry)
✅ **Real-time critical**: Emergency stops, motor commands
✅ **Many subscribers**: Multiple nodes need same data
✅ **One-way**: Publisher doesn't care about response
✅ **Latest value matters**: Old data can be dropped

**Example**: Camera images at 30 Hz—subscribers just want latest frame.

### Use Services When:

✅ **Low frequency**: Occasional operations (reset, save map)
✅ **Need confirmation**: Must know if operation succeeded
✅ **Compute result**: Calculate path, solve IK, query state
✅ **One-to-one**: Only one server should handle request
✅ **Blocking acceptable**: Client can wait for response

**Example**: "Save current map to disk"—need confirmation it saved.
...
🎓 **Services** provide **synchronous request-response** communication, complementing asynchronous topics.

🎓 **`.srv` files** define service interfaces with request (above `---`) and response (below) sections.

🎓 **Service servers** use `create_service()` and implement callback returning `response`.

🎓 **Service clients** use `create_client()`, `call_async()`, and `spin_until_future_complete()` for blocking calls.

🎓 **Validation** in service callbacks ensures data integrity (check ranges, IDs, permissions).

🎓 **Decision matrix**: Topics for streaming/real-time, Services for queries/commands/confirmations.
...
**Continue to** → [Chapter 1.5: URDF](./chapter-1-5-urdf.md)
...
**Next Chapter**: [1.5 URDF](./chapter-1-5-urdf.md)
