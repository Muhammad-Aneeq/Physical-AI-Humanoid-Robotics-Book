...
In Chapter 1.1, you observed ROS 2 in action with TurtleSim—watching nodes communicate through topics. Now it's time to **create your own nodes** and build the foundation for every ROS 2 application: the **publish-subscribe pattern**.
...
**Analogy**: A speaker broadcasting on a radio frequency—anyone tuned to that frequency receives the signal.
...
5. **main()**: Entry point—initialize ROS 2, run node, cleanup
...
**Congratulations!** You've created your first ROS 2 publisher. 🎉
...
**Try this**:
1. Run only talker → See `/talker` publishing to `/chatter` (no subscriber)
2. Start listener → Arrow connects to `/listener`
3. Stop talker → `/listener` still subscribes, but no messages arrive
...
🎓 **Publish-subscribe pattern** decouples nodes, enabling scalable, asynchronous communication in robotics.

🎓 **Publishers** send messages to topics without knowing who receives them; **subscribers** receive messages via **callback functions**.

🎓 **rclpy** is the ROS 2 Python library providing `Node`, `create_publisher()`, `create_subscription()`, `create_timer()`.

🎓 **ROS 2 CLI tools** (`topic list/info/echo/hz/pub`) are essential for debugging communication issues.

🎓 **rqt_graph** visualizes the computation graph, showing nodes, topics, and message flow.

🎓 **colcon build** compiles ROS 2 packages; **source install/setup.bash** makes them discoverable.
...
**Continue to** → [Chapter 1.3: Custom Messages](./chapter-1-3-custom-messages.md)
...
**Next Chapter**: [1.3 Custom Messages](./chapter-1-3-custom-messages.md)
