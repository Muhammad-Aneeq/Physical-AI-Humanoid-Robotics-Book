...
**RViz2** (ROS Visualization 2) is a 3D visualization tool that renders robot models, sensor data, transforms, and more—essential for debugging and developing robotics systems.
...
✅ **RViz2 configuration** to visualize robot models
✅ **Animated robotic arm** with controllable joints
✅ **TF tree** understanding for debugging transforms
✅ **Launch file** to start RViz2 with robot automatically
...
1. **URDF** (robot description) → Defines geometry, links, joints
2. **Joint states** (current joint angles) → Defines robot pose
...
**Workflow**:
```
URDF → robot_state_publisher → TF transforms → RViz2
                ↑
...
**Try this**:
1. Move **shoulder_joint** slider → Arm rotates
2. Move **elbow_joint** slider → Forearm bends
...
**Views → Zero**: Resets camera to default position.
...
🎓 **RViz2** is ROS 2's 3D visualization tool for robot models, sensor data, and transforms.

🎓 **robot_state_publisher** converts URDF + joint states → TF transforms for visualization.

🎓 **joint_state_publisher_gui** provides sliders to control joint angles interactively.

🎓 **TF tree** shows coordinate frame relationships—use `tf2_echo` and `view_frames` for debugging.

🎓 **RViz config files** (`.rviz`) save display settings for reuse across sessions.

🎓 **Displays**: RobotModel (robot), TF (frames), Grid (reference), LaserScan, Image, etc.

---

## What's Next?

You've visualized robots in RViz2—now it's time to **put it all together**!
...
**Continue to** → [Chapter 1.7: Mini-Project](./chapter-1-7-mini-project.md)
...
**Next Chapter**: [1.7 Mini-Project](./chapter-1-7-mini-project.md)
