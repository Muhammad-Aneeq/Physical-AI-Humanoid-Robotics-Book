...
**URDF (Unified Robot Description Format)** is the answer—an XML format that describes robot morphology: geometry, kinematics, dynamics, and visual appearance. It's the foundation for simulation (Gazebo, Isaac Sim) and visualization (RViz2).
...
✅ **simple_robot.urdf**: Two-wheeled robot with box chassis
✅ **robotic_arm.urdf**: 2-DOF arm with revolute joints
✅ **humanoid.urdf**: Humanoid with torso, head, 2 arms, 2 legs
...
✅ **Simulation**: Gazebo and Isaac Sim load URDFs for physics
✅ **Visualization**: RViz2 renders robot models from URDF
✅ **Motion Planning**: MoveIt uses URDF for inverse kinematics
✅ **Standard Format**: Every ROS 2 robot has a URDF
...
A **joint** connects two links (parent → child).
...
```xml
<origin xyz="0 0 0" rpy="-1.5708 0 0"/>  <!-- -π/2 radians = -90° -->
```
...
Kinematic tree: `base_link → link1 → link2` ✅
...
```
torso (root)
├── head (neck_joint: fixed)
├── left_upper_arm (left_shoulder_joint: revolute)
│   └── left_forearm (left_elbow_joint: revolute)
└── right_upper_arm (right_shoulder_joint: revolute)
    └── right_forearm (right_elbow_joint: revolute)
```
...
❌ Bad: `link1`, `link2`, `j1`, `j2`
✅ Good: `torso`, `left_upper_arm`, `left_shoulder_joint`

### 2. Use Meters for Units

URDF always uses **meters**, **kilograms**, **seconds** (MKS).

❌ `<box size="600 400 200"/>` (wrong)
✅ `<box size="0.6 0.4 0.2"/>` (correct)
...
🎓 **URDF** describes robot morphology using XML with links (rigid bodies) and joints (connections).

🎓 **Links** have three representations: visual (rendering), collision (physics), inertial (dynamics).

🎓 **Joint types**: fixed, revolute, continuous, prismatic—each suited for different mechanisms.

🎓 **Kinematic tree**: URDF forms a tree structure with one root and no loops.

🎓 **Coordinate system**: ROS uses X=forward, Y=left, Z=up; rotations are roll-pitch-yaw in radians.

🎓 **Validation**: Use `check_urdf` to catch syntax errors before loading in RViz2 or Gazebo.

---

## What's Next?

You've described robots in URDF—now it's time to **see them**!
...
**Continue to** → [Chapter 1.6: RViz2](./chapter-1-6-rviz.md)
...
**Next Chapter**: [1.6 RViz2](./chapter-1-6-rviz.md)
