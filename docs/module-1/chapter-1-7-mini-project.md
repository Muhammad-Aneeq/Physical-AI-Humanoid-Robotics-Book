...
In this mini-project, you'll create a **sensor tower**—a multi-sensor system that monitors environmental conditions (temperature, humidity, light) and displays data in real-time.
...
class SensorMonitor(Node):
    ...
    def temperature_callback(self, msg):
        ...
        self.get_logger().info(
            f'[TEMP] {msg.sensor_id}: {msg.temperature:.2f}°C'
        )
    ...
    def print_summary(self):
        ...
        self.get_logger().info(f'  Temperature:  {self.latest_temp:.2f}°C (received {self.temp_count} msgs)')
...
```
[INFO] [sensor_monitor]: [TEMP] temp_sensor_01: 23.45°C
...
[INFO] [sensor_monitor]: [TEMP] temp_sensor_01: 21.89°C
...
[INFO] [sensor_monitor]: ============================================================
[INFO] [sensor_monitor]: SENSOR TOWER SUMMARY (Last 10 seconds)
[INFO] [sensor_monitor]:   Temperature:  22.34°C (received 10 msgs)
...
└────────────────────┘  │
                        │  /sensors/temperature
┌────────────────────┐  ├──────────────────> ┌─────────────────┐
│ /humidity_sensor   │──┤  /sensors/humidity  │ /sensor_monitor │
└────────────────────┘  │                     └─────────────────┘
                        │  /sensors/light
┌────────────────────┐  │
│ /light_sensor      │──┘
...
- ✅ 3 custom messages created and built (15 points)
- ✅ Temperature sensor publishes at 1 Hz (10 points)
- ✅ Humidity sensor publishes at 2 Hz (10 points)
- ✅ Light sensor publishes at 5 Hz (10 points)
- ✅ Monitor node receives all 3 sensor types (15 points)

**Code Quality (20 points)**:
- ✅ Proper Python style (PEP 8) (5 points)
- ✅ Meaningful variable names (5 points)
- ✅ Logging messages informative (5 points)
- ✅ Error handling (try/except where needed) (5 points)

**Verification (10 points)**:
- ✅ `rqt_graph` screenshot showing system topology (5 points)
- ✅ `ros2 topic hz` output showing correct rates (5 points)

**Documentation (10 points)**:
- ✅ README.md with setup instructions (5 points)
- ✅ Comments in code explaining logic (5 points)
...
🎓 **Integration**: Combined publishers, subscribers, custom messages, and multi-node systems.

🎓 **Real-world patterns**: Sensor data streaming is fundamental to robotics (cameras, LiDAR, IMU all follow this pattern).

🎓 **Debugging**: Used `rqt_graph`, `ros2 topic echo/hz`, and node logs to verify system behavior.

🎓 **Scalability**: Pub/sub pattern allows adding more sensors (pressure, CO2, motion) without changing monitor.

🎓 **Module 1 mastery**: Demonstrated understanding of all core ROS 2 concepts.
...
- ✅ Design custom message types for application-specific data
- ✅ Build service servers and clients for request-response patterns
- ✅ Model robots with URDF (links, joints, geometry)
- ✅ Visualize robots in RViz2 and debug with TF trees
- ✅ Build complete multi-node systems
